...
class SUS:
    ...
    def __init__(
        ...
        lines: list[str],
        note_size=1.0,
        note_host='https://strage.kyonchan.net/sus2svg',
        **kwargs,
    ) -> None:
    ...
    def __getitem__(self, key: slice) -> svgwrite.Drawing:
        ...
        def add_slide_path(note: Slide):
            ...
            slide_paths.append(svgwrite.path.Path(
                ...
                #class_='slide' if not note.head.tap or note.head.tap.type == 1 else 'slide-critical', stroke='red', fill="#bcdcca"),
                class_=class_name,
                stroke=color,
                fill=color)
        ...
        def add_tick_text(note: Note, next: Union[Note, None] = None):
            y = self.pixel_per_second * self.score.get_time_delta(note.bar, bar_to) + self.padding
    ...
    def export_xdraw(self, file_name, style_sheet=""):
        n_bars = math.ceil(self.score.notes[-1].bar - 1e-3)
    ...
    def export_ydraw(self, file_name, style_sheet=""):
        n_bars = math.ceil(self.score.notes[-1].bar - 1e-3)
        drawings: list[svgwrite.Drawing] = []
